...
- Opportunities are "leads in pipeline" — not necessarily active transactions.
- GHL contacts API cursor does NOT filter by dateAdded, so weekly lead counts
  are estimated from opportunity creation dates (which DO filter correctly).
- Stale = open opp in an ACTIVE stage untouched 14+ days (excludes Cold/Unqualified/
  bulk-import pipelines which are expected to be dormant).
"""
...
# Stages that are "holding areas" — expected to be old, don't count as stale
DORMANT_STAGES = {
...
def get_opportunities_in_window(start_dt, end_dt, label=""):
    """Opps with createdAt in [start_dt, end_dt). Uses confirmed-working cursor."""
    opps = []
        ...
        for o in batch:
            ...
            if cat:
                try:
                    ...
                    pass
        print(f"  {label} opps p{_page+1}: {len(batch)} fetched, {len(opps)} in window", flush=True)
...
def get_all_open_opportunities():
    """All non-won/non-lost opportunities. Paginates until exhausted."""
    opps = []
...
print("Fetching this week opps...", flush=True)
week_opps = get_opportunities_in_window(week_start_utc, week_end_utc, "this-wk")

print("Fetching prior week opps...", flush=True)
...
print(f"Summary: week_opps={len(week_opps)}, prior_opps={len(prior_opps)}, all_open={len(all_open)}", flush=True)
...
# New leads this week = new opportunities created (reliable metric)
new_leads_this_wk = len(week_opps)
new_leads_prior_wk = len(prior_opps)

# Break down new opps by pipeline this week
week_by_pipeline = defaultdict(int)
for o in week_opps:
    si = stage_info(o)
    week_by_pipeline[si["pipeline"]] += 1

prior_by_pipeline = defaultdict(int)
for o in prior_opps:
    si = stage_info(o)
    prior_by_pipeline[si["pipeline"]] += 1
...
stale_active = []      # active opps untouched 14+ days

stage_buckets = defaultdict(list)   # for pipeline-by-stage table
...
for o in all_open:
    ...
    if is_dormant_stage(si):
        ...
        last_ts = (o.get("lastStatusChangeAt") or
                   o.get("updatedAt") or
                   o.get("createdAt") or "")
        if last_ts:
            try:
                ldt = datetime.fromisoformat(last_ts.replace("Z", "+00:00"))
                idle = (now_utc - ldt).days
                if idle >= STALE_DAYS:
                    stale_active.append({
                        "id": o.get("id", ""),
...
stale_active.sort(key=lambda x: x["days_idle"], reverse=True)
open_pipeline_value = sum(float(o.get("monetaryValue") or 0) for o in active_open)

print(f"Active open: {len(active_open)}, dormant: {len(dormant_open)}, stale-active: {len(stale_active)}", flush=True)

# ─── Health score (based on reliable metrics only) ────────────────────────

total_new_opps = new_leads_this_wk
prior_new_opps = new_leads_prior_wk
...
if total_new_opps > 0:
    score += 10
if prior_new_opps > 0 and total_new_opps >= prior_new_opps:
    score += 5
if total_wins > 0:
    score += 15
...
# Penalise stale ACTIVE opps only (not dormant holding-area contacts)
score -= min(len(stale_active) * 5, 20)
...
if total_new_opps == 0:
    attention_items.append({
        ...
        "detail": "No leads entered any pipeline stage this week — check your lead sources",
        "color": "#ef4444",
    })

if stale_active:
    worst = stale_active[0]
    n = len(stale_active)
    attention_items.append({
        "icon": "&#9200;",
        "title": f"{n} active lead{'s' if n>1 else ''} untouched for {STALE_DAYS}+ days",
        "detail": f"Oldest: {worst['name']} — {worst['days_idle']}d in {worst['pipeline']} / {worst['stage']}",
        "color": "#f97316",
    })
...
if dormant_open:
    attention_items.append({
        ...
        "detail": "These are dormant — not counted as active deals. Consider a cleanup campaign.",
        "color": "#64748b",
    })

if prior_new_opps > 0 and total_new_opps < prior_new_opps * 0.5:
    attention_items.append({
        "icon": "&#128200;",
        "title": f"New pipeline entries down {int((1-total_new_opps/prior_new_opps)*100)}% vs prior week",
        "detail": f"{prior_new_opps} last week → {total_new_opps} this week",
        "color": "#f4b955",
    })
...
# Pipeline by stage (active only, top 12)
pipeline_rows = ""
stale_ids = {d["id"] for d in stale_active}
...
for label, deals in sorted(active_buckets.items(), key=lambda x: -len(x[1]))[:12]:
    ...
    stale_badge = (
        f' <span style="background:#f9731618;color:#f97316;font-size:10px;padding:2px 6px;border-radius:10px;font-weight:600">{n_stale} stale</span>'
        if n_stale else ""
    )
...
if not stale_rows:
    stale_rows = '<tr><td colspan="5" style="padding:20px;color:#22c55e;text-align:center">&#10003; No stale active leads</td></tr>'
...
<div class="wrap">
  ...
  <div class="score-card">
    ...
    <div class="note">Score based on: new pipeline activity, wins, active deal health, and pipeline value. Cold/Unqualified stages excluded.</div>
  </div>
  ...
  <div class="kpi-grid">
    <div class="kpi">
      ...
      <div class="wow">""" + delta_badge(total_new_opps, prior_new_opps) + """</div>
    </div>
    <div class="kpi">
    ...
    <div class="kpi">
      ...
      <div class="wow"><span style="color:#334155;font-size:11px">excl. cold/unqualified</span></div>
    </div>
    <div class="kpi">
  ...
  <div class="section">
    ...
    <p style="font-size:12px;color:#475569;margin-bottom:12px">Open deals in active working stages (Cold, Unqualified, and holding stages shown separately below)</p>
    <table>
      <thead><tr>
    ...
    <p class="caveat">""" + str(len(dormant_open)) + """ contacts in Cold / Unqualified / holding stages not shown above. These are expected to be dormant.</p>
  </div>

  <div class="section">
    <h2>&#9200; Needs Follow-Up (""" + str(STALE_DAYS) + """+ Days Untouched)</h2>
    <p style="font-size:12px;color:#475569;margin-bottom:12px">Active-stage leads that haven&rsquo;t moved in """ + str(STALE_DAYS) + """+ days — these people need a call or text</p>
    <table>
      <thead><tr>
        <th>Name</th>
        <th>Pipeline</th>
  ...
  <div class="section">
    ...
    <p style="font-size:12px;color:#475569;margin-bottom:12px">Opportunities created this week by pipeline (reliable metric — uses createdAt timestamp)</p>
    <table>
      <thead><tr>
        <th>Pipeline</th>
  ...
  <div class="section">
    ...
    <div style="display:flex;gap:20px;margin-top:14px;padding-top:12px;border-top:1px solid #0e1a2e;flex-wrap:wrap">
      ...
      <div style="font-size:13px;color:#64748b">Prior week entries: <strong style="color:#94a3b8">""" + str(prior_new_opps) + """</strong></div>
    </div>
  </div>
...
if GITHUB_OUTPUT:
    with open(GITHUB_OUTPUT, "a") as go:
        ...
        go.write(f"stale_active={len(stale_active)}\n")
